...
> **Plan/Spec 边界提醒**：Plan 回答"改什么、为什么改、改哪里"——写到让 Review 能判断方向对不对、有没有遗漏维度的程度（文件路径 + Task 验收标准 + 架构维度全覆盖）。**不要**在 Plan 中写完整 TS 类型定义、WHEN-THEN 场景、精确函数签名——那是 Spec 的职责。详见 [《ADD开发工作路径与文档协同规范》§8.1.1](/home/xmm/ai/resources/docs/knowledge/01-架构/《ADD开发工作路径与文档协同规范》.md)。
...
- **主导 AI**: {AI 助手标识}
- **关联文档**:
  - ADD Route: `.codex/plans/{YYYY-MM}/{DD}/{需求域名}-{核心内容}-add-route-v{版本}.md`
  - Handoff: `.codex/plans/{YYYY-MM}/{DD}/{需求域名}-{核心内容}-handoff-v{版本}.md`
  - Review: `.codex/reviews/{需求域名}-review-v{版本}.md`
- **ADD-7 审计策略**:
...
> 详细子任务拆解 + 验证证据见 `.codex/specs/{name}/tasks.md`（含 Plan→Task 映射表）。
...
| 文档 | 路径 |
|------|------|
| ADD Route | `.codex/plans/...` |
| Handoff | `.codex/plans/...` |
| Review | `.codex/reviews/...` |
| Spec | `.codex/specs/{name}/spec.md` |
| Tasks | `.codex/specs/{name}/tasks.md` |
| Checklist | `.codex/specs/{name}/checklist.md` |
