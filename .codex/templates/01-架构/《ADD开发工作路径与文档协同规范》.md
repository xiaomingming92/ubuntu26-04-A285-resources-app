...
> ⚠️ **偏差声明**：
> - 审计 API 使用 `agentAudit("PHASE", message, extra)`（event-based），需实现三通道输出（console + file + AuditLog 表）。add-coder 配套示例项目提供了审计日志器参考实现，见 `docs/resources/knowledge/02-规范/《ADD可审计开发范式案例参考》.md`。
...
│            │  产物：.codex/plans/{需求域名}-plan-v{n}.md                        │
│            │        .codex/plans/{需求域名}-add-route-v{n}.md                   │
│            │        .codex/reviews/{需求域名}-review-v{n}.md                    │
...
│            │  产物：.codex/reviews/{需求域名}-review-implementation-v{n}.md       │
│            │        .codex/reviews/{需求域名}-review-runtime-v{n}.md              │
├────────────┼─────────────────────────────────────────────────────────────────┤
│  Step 4    │  审计数据验证                                                      │
...
| **日志代理用户 ID** | 所有 `AuditLog` 写入及业务表 `createdBy`/`createdById` 字段统一使用项目日志代理用户 ID 函数（禁止硬编码 `"system"` 等字符串）。详见 `.codex/rules/project_rules.md` ADD-4 §日志代理用户 ID |
...
| `.codex/plans/` | 需求方案 + 任务拆分 + 轮间交接手册 | 开发内部 |
| `.codex/reviews/` | 方案评审 + 逐轮 spec 评审 | 开发内部 |
| `.codex/specs/` | 每轮 spec + tasks + checklist（三件套） | 开发内部 |
| `.codex/rules/` | 项目规则文件（权威约束） | 开发内部 |
| `.codex/skills/` | SKILL 行为定义（AI 助手的标准行为模式） | 开发内部 |
...
- `plans/` + `reviews/` + `specs/` 三者都在 `.codex/` 下：它们是 ADD 开发流程的产物，面向 AI 和开发者，不属于公开文档
...
```
resources-多轮对话能力专家链路优化统一状态管理-plan-v1.md
resources-多轮对话能力专家链路优化统一状态管理-7轮原子事务拆分-add-route-v1.md
resources-多轮对话能力专家链路优化统一状态管理-7轮原子事务交接-handoff-v1.md
resources-多轮对话能力专家链路优化统一状态管理-方案评审-review-v1.md
resources-多轮对话能力专家链路优化统一状态管理-round1-类型收敛-thinkingLevel路由-spec-review-v1.md
resources-多轮对话能力专家链路优化统一状态管理-round2-响应策略裁决-spec-review-v1.md
resources-多轮对话能力专家链路优化统一状态管理-round3-专家注册分析上下文-spec-review-v1.md
```

### 命名带来的好处

1. **可追溯**：从文件名直接回答"什么需求、第几轮、什么工作、第几版本"
2. **可聚合**：`ls resources-*` 一条命令捞出全部关联文件
...
| 目录 | 对应的工作 |
|------|-----------|
| `resources-type-convergence-v1` | 第1轮：类型收敛 + thinkingLevel 路由 |
| `resources-response-strategy-v1` | 第2轮：响应策略裁决 |
| `resources-expert-registry-v1` | 第3轮：专家注册 + 分析上下文 |
| `resources-pipeline-integration-v1` | 第4轮：管线集成 |
| `resources-semantic-cache-v1` | 第5轮：语义缓存 |
| `resources-evolution-loop-v1` | 第6轮：演化闭环 |
| `resources-audit-pipeline-v1` | 第7轮：审计管线 |
...
```
需求理解 + 任务拆分  →  .codex/plans/      （plan + add-route + handoff）
    │
    ↓
评审                  →  .codex/reviews/    （plan-review + roundN-spec-review）
    │
    ↓
...
## 六、实际案例：resources 7 轮原子事务

完整流程见：

| 阶段 | 文件 | 说明 |
|------|------|------|
| 需求方案 | `.codex/plans/resources-多轮对话能力专家链路优化统一状态管理-plan-v1.md` | 总体设计 |
| 拆分拓扑 | `.codex/plans/resources-多轮对话能力专家链路优化统一状态管理-7轮原子事务拆分-add-route-v1.md` | 7 轮依赖拓扑 |
| 方案评审 | `.codex/reviews/resources-多轮对话能力专家链路优化统一状态管理-方案评审-review-v1.md` | 可行性验证 |
| 交接手册 | `.codex/plans/resources-多轮对话能力专家链路优化统一状态管理-7轮原子事务交接-handoff-v1.md` | 轮间输入输出 |
| 第1轮 spec | `.codex/specs/resources-type-convergence-v1/` | 类型收敛三件套 |
| 第1轮评审 | `.codex/reviews/resources-多轮对话能力专家链路优化统一状态管理-round1-类型收敛-thinkingLevel路由-spec-review-v1.md` | spec 人工 review |
| ... | ... | 第2-7 轮同理 |
...
- **`.codex/rules/project_rules.md`** 中的 ADD-8 规则（权威约束，AI 助手强制执行）
...
| **禁止项** | 架构级**设计约束**："禁止删除全局 resources collection" | 实现级**操作禁止**："禁止一次性迁移超过 3 个 Expert" |
...
策略文件位于 `.codex/sync-policy.json`，重型 add-route 模板已内置对应的 §0.8 / §4.6 闸门段落。
