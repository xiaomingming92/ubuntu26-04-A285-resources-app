...
> **完整词汇表已独立维护**：`.codex/vocabulary/add-governance-vocabulary.md`
> **六类分类**: A.文档类型 / B.ADD阶段 / C.门禁/闸门 / D.MCP工具 / E.Skills/Subagents / F.核心概念
> **消费者**: IDE 侧 LLM + 未来治理 AI。领域触发词汇（面向 resources 运行时用户 LLM）另有独立路径。

### 理论→实践映射

> 完整结构化映射：`.codex/rules/theory-practice-map.toml`（可被脚本解析验证）
...
SKILL 文件位于 `.codex/skills/session-init/SKILL.md`，包含 4 个步骤：
...
SKILL 文件位于 `.codex/skills/add-paradigm/SKILL.md`，包含 10 个阶段（Step 0 - Step 9），每个阶段包含若干子步骤：
...
**此外，ADD 工作流的核心产物由 `.codex/templates/` 下的 13 个模板定义**，这些模板不是参考资料，而是每次变更必须产出的文档骨架。分析变更影响范围时，必须同步确认需要创建/更新哪些模板产物：
...
错误示例：
- ❌ `/home/xmm/ai/resources/src/middleware.ts`（绝对路径，Linux 下不可移植）
- ❌ `C:\Users\xxx\resources\src\middleware.ts`（绝对路径，Windows 下不可移植）

正确示例：
- ✅ `src/middleware.ts`（resources workspace 内）
- ✅ `agrisynapse/src/api/agent/types.ts`（跨项目时带项目名前缀）
- ✅ `.codex/plans/resources-agrisynapse-integration-plan-v1.md`（.codex 内文件）
...
| `.codex/specs/` | 每轮 spec + tasks + checklist（三件套） | 开发内部 |
| `.codex/templates/` | 文档模板（plan/spec/tasks/checklist/review） | 开发内部 |
| `.codex/rules/` | 项目规则文件 | 开发内部 |
| `.codex/skills/` | SKILL 行为定义 | 开发内部 |
| `.codex/scripts/` | 工具脚本 + MCP 服务器 | 开发内部 |
...
```
需求理解 + 任务拆分 → .codex/plans/      （plan + execution + handoff）
    ↓
Review（强制关卡）    → .codex/reviews/    （plan-review + roundN-spec-review）
    ↓
Spec 执行              → .codex/specs/       （三件套：spec + tasks + checklist）
```
...
- 每轮的 Review 文件路径：`.codex/reviews/{需求域名}-round{N}-{核心内容}-spec-review-v{版本}.md`

### 文档产出物模板

ADD 开发流程产生五类文档，模板文件位于 `.codex/templates/`：

| 模板 | 文件路径 | 说明 |
|------|---------|------|
| Plan | `.codex/templates/plan-template.md` | 元信息 + 背景目标 + 方案选型 + 架构设计 + 实施步骤 + 验收标准 + 关联文档 |
| Spec | `.codex/templates/spec-template.md` | Why + What Changes + Impact + Requirements(WHEN/THEN) |
| Tasks | `.codex/templates/tasks-template.md` | Phase → Task → SubTask 层级 |
| Checklist | `.codex/templates/checklist-template.md` | Phase 检查项 + ADD 规则合规检查 |
| Review | `.codex/templates/review-template.md` | 元信息 + 问题复现 + 方案对比 + 决策结论 + 影响评估 |
| Handoff（单轮） | `.codex/templates/handoff-single-round-template.md` | 单轮变更 Handoff 基础格式（9 章节） |
| Handoff（多轮） | `.codex/templates/handoff-multi-round-template.md` | 多轮原子事务 Handoff（全局结构 + 13 子章节 + 收敛规则 + 启动模板） |
...
| 5 | **spec 文件** | ✅ | `.codex/specs/{spec-name}/spec.md` + `tasks.md` + `checklist.md` 三件套路径 |
...
|---------|:---:|------|
| Plan（标准版/精简版） | `PLAN` | `.codex/plans/{YYYY-MM}/{DD}/` |
| Review（方案/实现） | `PLAN_REVIEW` | `.codex/reviews/` |
| Runtime Review | 不需要 | `[T]` 全部通过后直接写入 `.codex/reviews/` |
| Spec / Tasks / Checklist | 不需要 | 直接写入 `.codex/specs/` |
| Handoff | 不需要 | 直接写入 `.codex/plans/` |
...
**Plan、方案 Review 和实现 Review 在写入正式文件前，必须经过 HITL 审批。** 审批通过后，`update_hitl` 更新数据库与 `*.hitl.md` 状态，并在 `.codex/hitl/` 生成 `.tongyi-{planName}` 哨兵。pre-tool-use hook 对 Plan/方案 Review 执行哨兵门禁；实现 Review 由 `add-paradigm` Step 3.5 与 `status_hitl` 门禁约束。
...
AI 助手在**每次新对话启动时**，必须先执行 `.codex/skills/session-init/SKILL.md`（SKILL-1 会话初始化），作为**不可跳过的前置操作**。

`session-init` SKILL 的 Step 1 要求调用 `query_audit_logs` 工具查询开发操作审计记录，以恢复开发上下文。

**执行顺序**（新对话启动时的首个操作）：
1. 定位并执行 `session-init` SKILL（`.codex/skills/session-init/SKILL.md`）
...
MCP 服务器配置在 `.codex/mcp.json`，通过 `npx tsx .codex/scripts/mcp-server.ts` 启动。
Trae IDE 加载项目时自动连接 MCP 服务器。

### 附录

> 附录 A（resources ADD-0.3 AuditCallback 实现，含 LangChain `BaseCallbackHandler` 模式）已移至技术栈 profile：`.codex/rules/profiles/webapp-profile.md` §附录 A。未设置 webapp 技术栈时，该附录内容不构成约束。
