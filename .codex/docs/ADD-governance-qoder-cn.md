...
配置位置：`.qoder/hooks/*.mjs`（TS 源码 esbuild 烘焙零依赖产物）+ `.qoder/settings.json`（command 直调 node）——2026-08-14 hook node 化实态，bash 待退役
...
**重要约束**：Qoder 端 `prompt-submit.mjs`（TS 版，core `prompt-router.ts` 继承）已有完善的触发词路由系统（Layer 1 精准 P0 触发词 → Layer 2 开发关键词检测 → Layer 3 活跃 ADD 状态注入 + 验收幂等保护）。模板全文注入逻辑以**增量方式**插入 Layer 2 之后，禁止覆盖现有分流逻辑。
...
| `detect_active_add` 返回空 | node 版 PROJECT_DIR 未注入 | `tryResolveMagicDir` 注入链前置（env 优先 → 物理推导） |
