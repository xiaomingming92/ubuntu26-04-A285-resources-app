...
- [ ] [T] `npx add-coder init --adapter qoder` 生成正确的 `.codex/` 目录 — 证据: (待填写)|审计: (待填写)
  - [ ] [E] `.codex/settings.json` matcher 使用双套工具名（`Write|write_to_file`, `Edit|edit_file`, `Bash`）
  - [ ] [E] `.codex/hooks/` 下 12 个脚本 + `lib/` 存在
...
- [ ] [T] 已有 `.codex/settings.json` 时 `init` 不覆盖，展示 diff 并交互确认 — 证据: (待填写)|审计: (待填写)
...
> 参考: `policy-update-loop.ts` 的持续反馈闭环模式 + `.codex/reports/REPORT-WORKFLOW.md`。
...
> 2. **读取** `review-runtime-template.md`，复制为 `.codex/reviews/{project}-review-runtime.md`
>    - 替换占位符（标题、关联文档路径）
>    - §1 发现列表初始化为 "尚无运行时发现"
