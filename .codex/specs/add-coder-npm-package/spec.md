...
- 父 Plan: `.codex/plans/2026-07/08/add-coder-add-coder-npm-package-plan-v1.md`
- 依赖: 无
- 后续依赖: 无
...
- **WHEN** 模板文件包含项目特定值
- **THEN** 使用 `resources` 双花括号语法，而非 `${var}` 以避免与 Markdown 代码块中的 JS 模板字符串冲突
...
- **THEN** `templates/core/` 的所有文件 SHALL 同时写入 `.add/`、`.codex/`、`.claude/` 三个目标目录
- **AND** adapter renderers SHALL 只处理 adapter 专属文件（hooks/mcp.json/settings.json/sync-policy.json），不再单独渲染 core 内容

**改造步骤**：
1. `src/core/renderer.ts`：`renderCore()` 返回值改为输出三份文件的映射（`.add/` + `.codex/` + `.claude/`）
...
- **THEN** 生成正确的 `.codex/` 目录，hook 配置 matcher 适配双套工具名（`Write|write_to_file`, `Edit|edit_file`, `Bash`）
...
- **WHEN** 用户已有 `.codex/settings.json` 或 `.vscode/settings.json`
...
- **THEN** `.add/` `.codex/` `.claude/` 三目录均含完整 ADD 内容（agents/skills/templates/rules/vocabulary/scripts/hooks/plans/specs/reports/tools），`.vscode/` 含 IDE 配置

#### Scenario: plans/specs 示例

- **WHEN** 用户首次 init
- **THEN** `.codex/plans/` 含 add-coder 6轮范例，`.codex/specs/` 含 spec/tasks/checklist 范例
...
- **WHEN** 用户已有 `.codex/settings.json` 时执行 `init`
