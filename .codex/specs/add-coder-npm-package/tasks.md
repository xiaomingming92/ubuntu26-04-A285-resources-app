...
- [x] Task 1.1: 清理硬编码 + 参数化 core 模板 — 验证: `grep -r "farm.agent\|farm_secure_pass\|大田精准\|/home/xmm\|/Users/milkytea" templates/` 返回空
  - [x] 15 个 `.md` 模板（plan/spec/tasks/checklist/handoff/review/add-route 等）：`docs/大田精准耕播智能决策系统/` → `docs/`、`add-coder-*` → `resources-*`
  - [x] 15 个 `.schema.json`：检查确认无硬编码
  - [x] `skills/add-paradigm/SKILL.md`：`/home/xmm/ai/add-coder/` → `/home/xmm/ai/resources/`
  - [x] `skills/session-init/SKILL.md`：`add-coder-*` → `resources-*`
  - [x] `agents/add-flow-guardian.md`：检查确认
  - [x] `agents/add-orchestrator.md`：检查确认
  - [x] `rules/project_rules.md`：`add-coder` → `resources`、`src/agents/` → `src/agents/`
  ...
  - [x] `.codex/settings.json`：hook 脚本绝对路径 → `/home/xmm/ai/resources/.codex/hooks/`
  - [x] `.codex/mcp.json`：`DATABASE_URL` 硬编码密码 → `process.env.DATABASE_URL`
  - [x] `.codex/sync-policy.json`：检查确认
  - [x] `.codex/hooks/` 下 14 个 `.sh` + `lib/`：项目名提取逻辑 → 使用 `$CLAUDE_PROJECT_DIR` 或 `/home/xmm/ai/resources`
  - [x] `.vscode/` 下 4 个文件：MCP 配置中的项目特定路径 → `/home/xmm/ai/resources`
  - [x] `reports/` 下 7 个文件：`add-coder`、绝对路径 → `resources`、`/home/xmm/ai/resources`
...
- [x] Task 2.0: 模板目录重组 + 适配器架构搭建 — 验证: 新目录结构符合 §3.2，`find templates/ -name "*.ts" | wc -l` 返回 0
  ...
  - [x] 迁移 `.codex/hooks/`（14 个 .sh + lib/） → `templates/adapters/qoder/hooks/`
  - [x] 迁移 `.codex/settings.json` → `templates/adapters/qoder/settings.json`
  - [x] 迁移 `.codex/mcp.json` → `templates/adapters/qoder/mcp.json`
  - [x] 迁移 `.codex/sync-policy.json` → `templates/adapters/qoder/sync-policy.json`
...
- [x] Task 3.1: Qoder 适配器实现 — 验证: `npx add-coder init --adapter qoder` 生成正确的 `.codex/` 目录
...
- [x] Task 4.1: CLI 重写 — 验证: `npx add-coder init` 在空白项目中成功生成完整 ADD 模板
  ...
  - [x] `src/cli/detect.ts`：IDE 环境检测（扫描 `.codex/` `.claude/` `.vscode/` 存在性）
...
- [x] `npx add-coder init --adapter qoder` 生成 `.codex/` 目录
