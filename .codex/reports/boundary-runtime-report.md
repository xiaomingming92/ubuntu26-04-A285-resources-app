...
> 定义 `resources-runtime-report/`（运行时报告，按子系统分类写入）与 `reports/`（协同手工生成）的职责边界、数据流向和互操作协议。
>
...
│  (resources-runtime-report/gateway.md)  │    │  (reports/*.md)                 │
├─────────────────────────────────┤    ├─────────────────────────────────┤
...
appendRuntimeFinding() ─────────────────────────────┐
       ...
       │                                             │
       ├── 2. 写入 resources-runtime-report/gateway.md         │
       ...
       └── 3. 生成 reports/runtime-issue-{id}.md ────┘
              ...
              Reports 人工 triage
                    ...
                    └── 标记为 P1/P2 后 → Plan 流程
                                   ...
                                   下次请求跑通同一路径
                                   → resources-runtime-report/gateway.md 写回 verified_at
                                   → Issue 自动标记为 "已验证"
```
...
- **来源**: Gateway 运行时 (`resources-runtime-report/gateway.md`)
- **首次发生**: {timestamp}
- **发生次数**: 1
- **错误**: {errorMsg}
- **路径**: {method} {pathname}
- **堆栈**: (见 resources-runtime-report/gateway.md 对应条目)
...
- `resources-runtime-report/gateway.md` 中每条发现增加 `report-issue: RUNTIME-{id}` 引用
- `reports/combined-report.md` 中每条 Issue 增加 `runtime-finding: resources-runtime-report/gateway.md#L{line}` 引用
- 确保可双向追溯
...
| 问题 | 表现 | 修复方向 |
|------|------|---------|
| Runtime Report 无去重 | `resources-runtime-report/gateway.md` 452 行，90% 是重复条目 | `appendRuntimeFinding()` 增加去重逻辑 |
