...
├── resources-runtime-report/        ← 运行时报告目录（按子系统分类）
...
| 新建运行时报告子系统 | 新建 `resources-runtime-report/{subsystem}.md` | 使用 [runtime-report-template.md](../templates/runtime-report-template.md) |
...
1. combined-report.md 中 Issue 标记 ✅
       │
2. 验证方在 resources-runtime-report/gateway.md 中增加验证条目:
   ```
   ### 验证: RPT-{id}
   - 修复版本: commit {hash}
...
3. Runtime Report 子系统下次请求经过该路径:
   ├── 成功 → resources-runtime-report/gateway.md 追加 verified_at
   └── 失败 → resources-runtime-report/gateway.md 追加 regression_at → Issue 回退为 ❌
```
