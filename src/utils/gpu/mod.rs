...
impl Gpu {
    ...
    fn from_sysfs_path<P: AsRef<Path>>(path: P, i: usize) -> Result<Gpu> {
        ...
        // Some drivers expose hwmon under the DRM card's device directory,
        // others expose it directly under the card directory. Try both so
        // clock/temperature sensors are found on AMD APUs and older GPUs.
        let hwmon_patterns = [
            format!("{}/hwmon/hwmon?", sysfs_device_path.to_string_lossy()),
            format!("{}/hwmon/hwmon?", path.to_string_lossy()),
        ];
        for pattern in hwmon_patterns {
            for hwmon in glob(&pattern)?.flatten() {
