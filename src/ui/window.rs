    ...
    fn refresh_ui(&self, refresh_data: RefreshData) {
        ...
        for ((_, page), mut gpu_data) in gpu_pages.values().zip(gpu_data) {
            ...
            if !gpu_data.nvidia {
                ...
                // On AMD Raven/Picasso APUs (and some other old AMD/Intel GPUs)
                // /sys/class/drm/*/device/gpu_busy_percent can report a constant
                // 100% even when idle. Process-based fdinfo statistics are the
                // reliable signal, so prefer them instead of taking the maximum.
                let highest_gpu_fraction = processes_gpu_fraction.into();
                trace!(
