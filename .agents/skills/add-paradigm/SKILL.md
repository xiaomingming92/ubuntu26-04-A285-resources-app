...
**范式边界**（定义在 `.codex/rules/project_rules.md` ADD-0）：
- ADD 是开发阶段编程范式，不是运行时范式
- 反馈闭环消费者：IDE 中的 AI 助手 + 编程人员
- 运行时范式（集中裁决层/能力模型）是独立的下一步演化

**核心原则**（始终生效，定义在 `.codex/rules/project_rules.md`）：
- ADD-0：范式边界与消费者定义
...
3. **命名规范**：`{项目名}-{功能名}-plan-v1.md` → `.codex/plans/{YYYY-MM}/{DD}/`（按当天日期创建子目录）
...
> 4. TONGYI 通过后，生成完整 Review 写入 `.codex/reviews/`
...
**命名**：`{需求域名}-{核心内容}-add-route-v1.md` → `.codex/plans/{YYYY-MM}/{DD}/`（与 Plan 同目录）
...
**什么时候触发**：
- Plan Review 已生成（`.codex/reviews/{需求域名}-*review-v{n}.md` 存在）
...
1. **判定输入**：当前 adapter 自己的 `.codex/plans/`。各 adapter 的 Plan/Spec/Route/Review/HITL 相互独立，禁止跨 adapter fallback
...
- [ ] **DPS 上游文档质量闸门已通过**——调用 `check_dps({ planKeyword: "<Plan 核心关键词>" })`，DPS ≥ 80 方可进入 Step 1。未通过时回退补齐短板（Plan 粒度不足 / Review 覆盖维度缺失 / Specs Requirements 遗漏）。详见 [ADD 协同规范 §八](/home/xmm/ai/resources/docs/knowledge/01-架构/《ADD开发工作路径与文档协同规范》.md#八双质量闸门dps-与-rahs)
...
> **目标**：确保 `.codex/specs/`、`.codex/reviews/` 下的 spec/review/handoff 文件遵循统一的命名和格式约定，使后续 AI Session 能快速定位和恢复上下文。

> **全局文档编辑规则（A.0）**：修改 Plan、Spec、Handoff、tasks、checklist 中已有内容时，MUST 遵循增量修订格式——旧内容 `~~删除线~~` → 新内容 `[日期 修订: 原因]`，禁止直接覆盖。详见 `.codex/rules/project_rules.md` §文档增量修订规则。
...
| 文档类型 | 命名规则 | 示例 | 存放位置 |
|---------|---------|------|---------|
| 开发任务（specs 三元组） | `项目名-任务名/` | `resources-response-strategy/` | `.codex/specs/` |
| review 文件 | `项目名-任务名-round{N}-review.md` | `resources-response-strategy-round2-review.md` | `.codex/reviews/` |
| handoff 文件 | `项目名-需求名-handoff.md` | `resources-co-agent-handoff.md` | `.codex/plans/{YYYY-MM}/{DD}/`（与 Plan 同目录） |

**命名规则说明**：

- **项目名**：当前仓库的项目标识，本项目为 `resources`
...
检查 `.codex/specs/{task}/checklist.md` 中的所有检查项：
...
读取 `review-implementation-template.md`，将实现审查发现整理为 `dimensions`，调用 `create_hitl({ planName: "{name}-review-implementation", type: "PLAN_REVIEW", dimensions })`。`create_hitl` 自动生成 `*.hitl.md` 审批提案并写入 HITL 记录；人类审核后调用 `update_hitl` 完成拍板。通过 `status_hitl` 确认状态为 `TONGYI` 后，方可逐条展开详细分析并写入 `.codex/reviews/`；若为 `BOHUI`，修订发现后调用 `create_hitl` 新建 round。
...
2. 复制为 `.codex/reviews/{project}-review-runtime.md`
3. 替换占位符（标题、关联文档路径）
...
- [ ] 失败路径审计等价
- [ ] 数据库审计字段有数据
- [ ] **RAHS 下游执行健康度已检查**——调用 `check_rahs({ planKeyword: "<Plan 核心关键词>" })`，RAHS ≥ 90 方可进入 Step 5。若 < 70 注意力漂移严重，强制返工 Step 3。详见 [ADD 协同规范 §八](/home/xmm/ai/resources/docs/knowledge/01-架构/《ADD开发工作路径与文档协同规范》.md#八双质量闸门dps-与-rahs)
...
> **收敛确认后、handoff 生成前，MUST 记录 ROUND_CLOSED。** ADD-7 原则见 `.codex/rules/project_rules.md` §ADD-7。
...
5. **写入位置**：`{项目名}-{需求名}-handoff.md` → `.codex/plans/`
...
读取 `.codex/templates/report-handoff-template.md`，按模板格式在 handoff 中追加 Report Closure 章节。
